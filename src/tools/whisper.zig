...
fn fixtureDir() []const u8 {
    return build_options.fixture_dir orelse default_fixture_dir;
}
...
fn format_range(
    ...
    comptime {
        if (format.len == 0) @compileError("format must not be empty");
        if (end < start) @compileError("format_range requires end >= start");
    }
    var result: [end - start][]const u8 = undefined;
    inline for (&result, 0..) |*ptr, i| {
        ptr.* = std.fmt.comptimePrint(format, .{start + @as(u32, @intCast(i))});
    }
    return result;
}

fn format_range_float(comptime count: u32) [count][]const u8 {
    @setEvalBranchQuota(1_000_000);
    comptime if (count == 0) {
        @compileError("count must be positive");
    };
    ...
    inline for (&result, 0..) |*ptr, i| {
        ...
        const frac = @as(
            u32,
            @intFromFloat((seconds - @as(f32, @floatFromInt(whole))) * 100.0 + 0.5),
        );
        if (frac < 10) {
...
pub const MultiHeadAttention = struct {
    ...
    pub fn init(key: []const u8, model_config: WhisperConfig, mlx_config: *mlx.MLXConfig) !*Self {
        ...
        const q_key = try self.base.alloc_join(key, "q_proj");
        self.q_proj = try mlx.Linear.init(q_key, true, null, mlx_config);
        const k_key = try self.base.alloc_join(key, "k_proj");
        self.k_proj = try mlx.Linear.init(k_key, false, null, mlx_config);
        const v_key = try self.base.alloc_join(key, "v_proj");
        self.v_proj = try mlx.Linear.init(v_key, true, null, mlx_config);
        const o_key = try self.base.alloc_join(key, "out_proj");
    ...
    pub fn forward(self: *Self, result: *mlx.Array, x: mlx.Array, xa: ?mlx.Array, mask: ?mlx.Array, kv_cache: ?*mlx.KVCache) !void {
        ...
        try mlx.reshape_to_heads(&k, k, self.n_heads, self.head_dim, self.base.stream);
        try mlx.reshape_to_heads(&v, v, self.n_heads, self.head_dim, self.base.stream);
...
pub const AudioEncoder = struct {
    ...
    pub fn init(key: []const u8, model_config: WhisperConfig, mlx_config: *mlx.MLXConfig) !*Self {
        ...
        self.* = .{
            ...
            .conv1 = undefined,
            .conv2 = undefined,
            .positional_embedding = mlx.array_new(),
            .layers = undefined,
            .layer_norm = undefined,
        };
        const pe_key = try self.base.alloc_join(key, "embed_positions.weight");
        try mlx_config.weights_hash.put(pe_key, &self.positional_embedding);
        const conv1_key = try self.base.alloc_join(key, "conv1");
        self.conv1 = try mlx.Conv1d.init(conv1_key, 1, 1, 1, 1, true, mlx_config);
        const conv2_key = try self.base.alloc_join(key, "conv2");
        self.conv2 = try mlx.Conv1d.init(conv2_key, 2, 1, 1, 1, true, mlx_config);
        const ln_key = try self.base.alloc_join(key, "layer_norm");
        ...
        const layers_key = try self.base.alloc_join(key, "layers");
        for (0..@intCast(model_config.encoder_layers)) |i| {
            const i_key = try self.base.alloc_join(layers_key, i);
    ...
    pub fn deinit(self: *Self) void {
        self.conv1.deinit();
        self.conv2.deinit();
        mlx.array_free(self.positional_embedding);
...
pub const TextDecoder = struct {
    ...
    pub fn init(key: []const u8, model_config: WhisperConfig, mlx_config: *mlx.MLXConfig) !*Self {
        ...
        const embed_key = try self.base.alloc_join(key, "embed_tokens");
    ...
    pub fn forward(self: *Self, result: *mlx.Array, x: mlx.Array, xa: mlx.Array, mask: mlx.Array, kv_cache: *mlx.Cache, cross_kv_cache: *mlx.Cache) !void {
        ...
        var pos_embed_slice = mlx.array_new();
        defer mlx.array_free(pos_embed_slice);
        try mlx.slice(&pos_embed_slice, self.positional_embedding, &[_]c_int{ offset, 0 }, &[_]c_int{ offset + mlx.array_dim(x, 1), mlx.array_dim(self.positional_embedding, 1) }, &[_]c_int{ 1, 1 }, self.base.stream);
        ...
        try self.layer_norm.forward(result, result.*);
        try self.embed_tokens.as_linear(result, result.*);
    }
};
...
pub const Whisper = struct {
    ...
    pub fn init(key: []const u8, model_config: WhisperConfig, mlx_config: *mlx.MLXConfig) !*Self {
        ...
        const encoder_key = try self.base.alloc_join(key, "encoder");
        self.encoder = try AudioEncoder.init(encoder_key, model_config, mlx_config);
        const decoder_key = try self.base.alloc_join(key, "decoder");
    ...
    pub fn forward(self: *Self, output_tokens: []u32, raw: mlx.Array) ![]u32 {
        var enc = mlx.array_new();
        defer mlx.array_free(enc);
        ...
        var toks = try mlx.array_new_data(
            &[_]u32{ 50258, 50360, 50365 },
            &[_]c_int{ 1, 3 },
            mlx.UINT32,
        );
        defer mlx.array_free(toks);
        var logits = mlx.array_new();
        defer mlx.array_free(logits);
        var mask = mlx.array_new();
        defer mlx.array_free(mask);
        var i: usize = 0;
        while (i < 446) : (i += 1) {
            try mlx.create_causal_mask(&mask, mlx.array_dim(toks, 1), kv_cache.offset, mlx.FLOAT16, self.base.stream);
            ...
            kv_cache.offset += mlx.array_dim(toks, 1);
            try mlx.array_set_data(
                &toks,
                &output_tokens[i],
                &[_]c_int{ 1, 1 },
                mlx.UINT32,
...
pub const Transcriber = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, model_path: []const u8) !Self {
        ...
        const model_config = try utils.load_config_json(WhisperConfig, allocator, model_path, true);
        ...
        const specials = [_][]const u8{
            "<|endoftext|>",
            "<|startoftranscript|>",
        } ++ format_range("<|_{d}|>", 0, 100) ++ [_][]const u8{
            "<|translate|>",
            "<|transcribe|>",
            "<|startoflm|>",
            "<|startofprev|>",
            "<|nospeech|>",
            "<|notimestamps|>",
        } ++ format_range_float(1501);
        const tokenizer_path = try std.fs.path.join(
            allocator,
            &.{ modelAssetsDir(), "multilingual.tiktoken" },
        );
        defer allocator.free(tokenizer_path);
        const tokenizer = try Tokenizer.init_from_tik_token(
            allocator,
            pattern,
            tokenizer_path,
            &specials,
        ...
        try mlx.load_model_safetensors(&mlx_config.weights_hash, model_path, mlx_config.stream);
    ...
    pub fn transcribe(self: *Self, audio_path: []const u8) ![]const u8 {
        const audio = try utils.load_audio(self.mlx_config.allocator, audio_path);
        defer self.mlx_config.allocator.free(audio);
        var mel_all = mlx.array_new();
        defer mlx.array_free(mel_all);
        var mel = mlx.array_new();
        defer mlx.array_free(mel);
        try getMel(
            &mel_all,
            audio,
            self.mlx_config.stream,
            self.mlx_config.allocator,
        );
        var new_tok = std.array_list.Managed(u32).init(self.mlx_config.allocator);
        defer new_tok.deinit();
        var i: c_int = 0;
        const mel_len = mlx.array_dim(mel_all, 1);
        ...
        const elapsed: f16 = @floatFromInt(now_ms() - start_time);
        const ntok = new_tok.items.len;
        const tps = @as(f16, @floatFromInt(ntok)) / (elapsed / 1000.0);
        std.debug.print("\n{d:.2} tokens-per-second ({d} tokens in {d:.2} ms)\n", .{ tps, ntok, elapsed });
        var filtered_tokens = std.array_list.Managed(u32).init(self.mlx_config.allocator);
...
fn getMel(
    ...
    var melp = mlx.array_new();
    defer mlx.array_free(melp);
    var hanp = mlx.array_new();
    defer mlx.array_free(hanp);
    const precomputed_joined = try std.fs.path.join(
        allocator,
        &.{ modelAssetsDir(), "whisper_precomputed.safetensors" },
    );
    defer allocator.free(precomputed_joined);
    const precomputed_path = try allocator.dupeZ(u8, precomputed_joined);
    defer allocator.free(precomputed_path);
    var safetensors = try mlx.Safetensors.load(precomputed_path, stream);
    defer safetensors.deinit();
    try mlx.load_array(&melp, "melp", null, &safetensors.weights);
    try mlx.load_array(&hanp, "hanp", null, &safetensors.weights);
    var audio = try mlx.array_new_data(
        audio_raw.ptr,
        &[_]c_int{@intCast(audio_raw.len)},
        mlx.FLOAT32,
    );
    defer mlx.array_free(audio);
    var threshold = mlx.array_new();
    defer mlx.array_free(threshold);
    try mlx.pad(&audio, audio, &[_]c_int{0}, &[_]c_int{0}, &[_]c_int{480000}, mlx.float(0.0), "constant", stream);
    var slice1 = mlx.array_new();
    defer mlx.array_free(slice1);
    var slice2 = mlx.array_new();
    defer mlx.array_free(slice2);
    ...
    try mlx.concatenate(&audio, &[_]mlx.Array{ slice1, audio, slice2 }, 0, stream);
    try mlx.as_strided(&audio, audio, &[_]c_int{ @divTrunc(mlx.array_dim(audio, 0) - 240, 160), 400 }, &[_]i64{ 160, 1 }, 0, stream);
    try mlx.multiply(&audio, audio, hanp, stream);
    try mlx.rfft(&audio, audio, 400, 1, stream);
    try mlx.slice(&audio, audio, &[_]c_int{ 0, 0 }, &[_]c_int{ mlx.array_dim(audio, 0), mlx.array_dim(audio, 1) }, &[_]c_int{ 1, 1 }, stream);
    ...
    try mlx.all_max(&threshold, audio, false, stream);
    ...
    try mlx.array_eval(result.*);
}

fn createSinusoids(result: *mlx.Array, length: c_int, channels: c_int, stream: mlx.Stream) !void {
    var inv_timescales = mlx.array_new();
    defer mlx.array_free(inv_timescales);
    var scaled_time = mlx.array_new();
    defer mlx.array_free(scaled_time);
    var sin_val = mlx.array_new();
    defer mlx.array_free(sin_val);
    var cos_val = mlx.array_new();
    defer mlx.array_free(cos_val);
    ...
    try mlx.einsum(
        &scaled_time,
        &[_]mlx.Array{ scaled_time, inv_timescales },
        "i,j->ij",
        stream,
    );
    try mlx.sin(&sin_val, scaled_time, stream);
    try mlx.cos(&cos_val, scaled_time, stream);
    try mlx.concatenate(result, &[_]mlx.Array{ sin_val, cos_val }, 1, stream);
...
pub fn main(init: std.process.Init) !void {
    ...
    _ = args.next();
    const default_audio_file = try std.fs.path.join(
        allocator,
        &.{ fixtureDir(), "audio/alive.mp3" },
    );
    defer allocator.free(default_audio_file);
    const audio_file: []const u8 = args.next() orelse default_audio_file;
...
test "Whisper transcription" {
    ...
    const model_name = "whisper-large-v3-turbo";
    const audio_file = try std.fs.path.join(
        allocator,
        &.{ fixtureDir(), "audio/alive.mp3" },
    );
